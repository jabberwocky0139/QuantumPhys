...
\begin{eqnarray}
  ...
  &&H_rv(\bx) = \varepsilon v(\bx)\label{2point}\\
...
\section{行き詰まりました}
いろいろ式を弄ってみたのですが, どうにも先に進めそうな気配がしてきません. 以下, その概要です.
\subsection{アイデアの概要}
\begin{figure}[H]
  \begin{center}
    \includegraphics[width = 2.5cm]{./EPS/ladder4.eps}
  \end{center}
\end{figure}
Ladder近似した$\Gamma$は上の破線部のような2点相互作用する部分の連結でできている. この2点相互作用部はSchr\"odinger方程式で表現できることから, \textbf{$\Gamma$はある2点相互作用を表現する波動関数でもって表現できる}と考えられる. 

\subsection{$\chi$の導入}
その波動関数とはつまり(\ref{2point})の$v$であると考えられるが, どのようにして置き換えを可能にするのか？ そもそも$H_r$は対角的ではないので$v$を求めるためには摂動計算に頼ることになるが, これをGreen関数で書き表せることに注目する:
\begin{eqnarray}
  v_n(\bx) &=& v^0_n(\bx) + \int d\bx'G_u(\bx, \bx')H_p(\bx')v_n(\bx')\\
  &=& v_n^0(\bx) + \int d\bx'U(\bx')G_u(\bx, \bx')v_n(\bx')\label{int_v}
\end{eqnarray}
$v$の引数は$\bx = \bx_1 - \bx_2$のような相対座標であることに注意. ひいては$U(\bx')$も2点相互作用を表していることになる. また$v_n$は完全系を構成できる:
\begin{eqnarray}
  \sum_n v_n(\bx)v_n^*(\by) = \delta(\bx -\by)
\end{eqnarray}
さてここで接触型を仮定しない$\Gamma$の積分方程式は
\begin{eqnarray}
  \nonumber  \Gamma(y_1, y_2; y_1', y_2') &=& -\frac{i}{2}U(y_1 - y_1')\delta(y_1 - y_2)\delta(y_1' - y_2')\\
  &&-\frac{i}{2}\int dzdz' U(y_1 - y_1')G_0(y_1, z)G_0(y_1', z')\Gamma(z, y_2; z', y_2')\label{int_Gamma}
\end{eqnarray}
のように書ける. もし$\Gamma$が
\begin{eqnarray}
  \Gamma \sim \sum_n v_n(\bx)v_n^*(\by)
\end{eqnarray}
であるような仮定をすると, (\ref{int_Gamma})の右辺は
\begin{eqnarray}
  &&-\sum_n\frac{i}{2}\int dzdz' U(y_1 - y_1')G_0(y_1, z)G_0(y_1', z')v_n(z)v_n^*(y_2)
\end{eqnarray}
であり, (\ref{int_v})の右辺の積分によって$v$に置き換えられる可能性が出てくる(非積分関数に$U$, $G$, $v$が含まれているから). その結果(\ref{int_Gamma})が恒等的に成り立てば, つまり$\Gamma$は$v$で記述できたことになります.

しかしそのような$\Gamma$の置き換えを見つけることは容易ではなく, 実際Fetterではまず(11.31)(11.33)にあるように, $Q$を経由してから$\chi$へ変換, $\chi$が$v$で書けるので最終的に$\Gamma$が$v$で書けたことになる, というシナリオを踏んでいる. $\chi$への変換というのは結局数学的な処理でしか無く, いろいろな定義ができそうである. 今回のdiscreteな$v$で$\Gamma$を置き換えるための適切な$\chi$の定義はどのようなものだろうか？

\subsection{適切な定義と仮定}
必要なのは, 適切な$\chi$の定義と適切な$\Gamma$の仮定である. Fetterでは$\Gamma$が変数を4つ持っているが, 重心運動量と相対運動量への変換と運動量保存則を用いて変数を$\bm{p}$と$\bm{P}$のように2つに落とし込んでいる. これと類似のテクニックを用いて$\Gamma$及び$\chi$を2変数の関数にして$v$の完全系を用いて(\ref{int_Gamma})を恒等的に成り立たせるような$\chi$の定義と$\Gamma$の仮定を見つけようとしましたが, いまだうまく行っていません. 重心座標・相対座標への変換はある程度有効だと思いましたが, (\ref{int_Gamma})と(\ref{int_v})の積分変数について微妙な食い違もあって難航しています.

どこかに勘違い等があって行き詰まっているのかもしれませんが, 何かコメントを頂けるとうれしいです. 
...
\begin{eqnarray}
  \chi(y_1, y_2) &=& -\frac{i}{2}\delta'(y_1, y_2)- \frac{i}{2}\int dzdz' U(z - z'){\cal G}_0(y_1, z)\chi(z, y_2)\label{int_chi}
\end{eqnarray}
さてここで$\chi$を$H_r$の固有関数で再構成したい. $v$とGreen関数の関係及び完全系は
\begin{eqnarray}
  &&v_n(\bx_1 - \bx_2) = v_n^0(\bx_1 - \bx_2) + \int d(\bm{z} - \bm{z}')G_0(\bx_1 - \bx_2, \bm{z} - \bm{z}')U(\bm{z} - \bm{z}')v_n(\bm{z} - \bm{z}')\label{int_v}\\
  &&\sum_n v_n(\bx)v_n^*(\by) = \delta(\bx -\by)
\end{eqnarray}
である. $U(z - z')$が摂動ハミルトニアンに対応している. Fetterと同様の流れを追うなら, $\chi$が$v_n$を用いて
\begin{eqnarray}
  \chi(\bx, \by) = A(\bx, \by) + B(\bx, \by)\sum_nv_n(\bx)v_n(\by)
\end{eqnarray}
と置いたときに(\ref{int_v})を用いて(\ref{int_chi})が恒等的に成立しているとうれしい.
\end{comment}
\newpage
%%-------------------- comment ---------------------------------------------------
%%-------------------- comment ---------------------------------------------------
