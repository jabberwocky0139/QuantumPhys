...
\end{eqnarray}
こういう, 「演算子ではないのに交換した時に符号がひっくり返るような数に対する代数」をGrassmann代数(外積代数)という. ただこれだとざっくりし過ぎなのでもう少し真面目に代数を構築する.
\subsection{Grassmann代数}
外積代数の名の通り, Grassmann代数はベクトル空間における外積と似たような構造を持っている. 外積は以下のルールに従う:
\begin{eqnarray}
  1.&& \bm{A}\times\bm{A} = 0\\
...
\end{eqnarray}
しかしながら, 4番目は内積が定義されていないと議論できないので, 1-3番目のルールに従う演算を「外積」と呼ぶことにし, 「$\Lambda$」という記号を用いることにする. ベクトル空間の元が4番目ののルールに従わないとき, 別のルールが生まれる:
\begin{eqnarray}
  1.&& \bm{A}\Lambda\bm{A} = 0\\
  2.&& \bm{A}\Lambda\bm{B} = -\bm{B}\Lambda\bm{A}\\
  3.&& \qty(a\bm{A} + b\bm{B})\Lambda\bm{C} = a\bm{A}\Lambda\bm{C} + b\bm{B}\Lambda\bm{C}\\
  4.&& \bm{A}\times\qty(\bm{B}\Lambda\bm{C}) = 
\end{eqnarray}
