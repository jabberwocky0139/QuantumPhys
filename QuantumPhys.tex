...
計算ノートのつもりで考えながら書いているのでテンポが悪いです. あまり意味のない記述もあります. スルーしてください.
\section{やりたいこと}
...
\subsection{ハミルトニアン}
まずは簡単のため, 3次元一様系で考える. それができたらトラップ系にも拡張したい. 秩序変数$\xi$と共役モード$\eta$が実であるような流れのない凝縮体を仮定する. また$\xi, \eta$が時間依存しないことを要請する. 以下, 各記号は元論文の表式に従う. 
...
\begin{eqnarray}
  ...
  \varphi_{ex} &=& \frac{1}{\sqrt{(2\pi)^3}}\int d\bm{k} \left[ u_ka_{\bm{k}} + v_k^*a^\dagger_{-\bm{k}} \right]e^{i\bm{k}\bm{x}}\label{plane}
\end{eqnarray}

非摂動ハミルトニアンのゼロモード部・励起部は
\begin{eqnarray}
  H_{u, z} &=& \frac{I - 4D}{2}P^2 + \frac{1}{2}AQ^4 - 2BQ^2 + CQP^2Q + \frac{1}{2}EP^4\\
...
\nonumber  T\exp[-\int_0^\beta ds H_{z, ex}(s)] &=& I -\int_0^{\beta}ds H_{z, ex}(s) + \frac{1}{2!}\int_0^\beta ds\int_0^\beta ds' T\qty[H_{z, ex}(s)H_{z, ex}(s')] + \cdots\\
 \therefore \ev{T\exp[-\int_0^\beta ds H_{z, ex}(s)]}_0 &\simeq& \ev{I -\int_0^{\beta}ds H_{z, ex}(s)}_0 =  1 - \int_0^\beta ds\ev{H_{z, ex}(s)}_0\label{Perturbed-thermo3}
%&=& 1 - e^{\beta\Omega_0}\sum_{mn}\int_0^\beta ds\bra{\Psi_m}\bra{n}e^{-\beta H_u}H_{z, ex}(s)\ket{n}\ket{\Psi_m}
\end{eqnarray}
ここで(\ref{Perturbed-thermo3})の積分内の期待値はゼロモード項と励起項に分離でき, 期待値が残るのは$\varphi_z, \varphi_{ex}$の2次及び$\varphi_{ex}$の4次のみである. 書き下すと
\begin{eqnarray}
  \nonumber  \ev{H_{z, ex}}_0 &=& \int d\bx\Bigl[\ev{\varphi_z^\dagger(s)\varphi_z^\dagger(s)}_z\ev{\varphi_{ex}(s)\varphi_{ex}(s)}_{ex} + 4\ev{\varphi_z^\dagger(s)\varphi_z(s)}_z\ev{\varphi_{ex}^\dagger(s)\varphi_{ex}(s)}_{ex}\\
...
計算しなければならないのは$Q^2(s), P^2(s)$. Heisenberg方程式は
\begin{eqnarray}
\nonumber  i\partial_t Q^2(t) &=& i(I-4D)(QP+PQ) + 2iCQ(QP+PQ)Q + 2iE\qty[P^2(QP+PQ) + (QP+PQ)P^2]\\
  i\partial_t P^2(t) &=& -iA(Q^2(QP+PQ) + (QP+PQ)Q^2) + 4iB(QP+PQ)-2iC(QP^3 + P^3Q)\label{Zeromode-Heisenberg}
\end{eqnarray}
となる. ハミルトニアンの形からHeisenberg方程式まで$a$-演算子とは全く異なる構造をしているので先程と同様の議論は成り立たない.  しかし, 今回$\xi$が実であることを仮定しているので, $Q$を奇数個含む期待値はゼロになる\footnote{例えば$\ev{Q^3}{\Psi} = \ev{QP+PQ}{\Psi} = 0$など. }. (\ref{Zeromode-Heisenberg})に現れる項は全て奇数個の$Q$を含んでいるので
\begin{eqnarray}
  i\partial_t \ev{Q^2(t)}{\Psi} &=&i\partial_t \ev{P^2(t)}{\Psi} = 0
\end{eqnarray}
である. つまり$a$-演算子同様, 時間依存性を気にする必要がない. もちろん, $\xi$が時間発展するような定式化の場合はこの限りではない.

\ul{ここの議論は少し不安でもあります. $\xi$が時間発展する場合としない場合の差がまだきちんと理解できていない}\\\ul{気がします. $\ket{\Psi_0}$が時間発展しないのはいいとしても, その励起である$\ket{\Psi_\nu}$は時間発展しないのか?など.}
...
\begin{eqnarray}
  H_{u, z}(p)\ket{\Psi} = \bra{p}\ket{\Psi}
\end{eqnarray}
...
時間依存性を考慮しなくて良いことがわかったのでさらに議論をすすめて数値計算が可能な形まで落としこむことにする. まず, (\ref{Hamiltonian-Exp})のゼロモード部について. 先ほど同様, $Q$の奇数次項を落とすと
\begin{eqnarray}
...
\nonumber \ev{\varphi_z^\dagger\varphi_z}_z &=& \xi^2\ev{Q^2}_z + \eta^2\ev{P^2}_z\\
...
\begin{eqnarray}
  ...
  &=& -\frac{1}{(2\pi)^2}\int_{+0}^\infty dk\frac{k^2\calM}{\omega_k}\qty(2n_k + 1)\label{BdG-Exp}
\end{eqnarray}
...
\begin{eqnarray}
  ...
  \begin{pmatrix}
    ...
    -\sqrt{{\cal L}_k - \omega_k}
  \end{pmatrix}\\
    {\cal M} = g(\xi^2 - V_a)\hspace{1cm}{\cal L}_k &=& \frac{\hbar^2}{2m}k^2 + {\cal M}\hspace{1cm}\omega_k = \sqrt{\varepsilon_k(\varepsilon_k + 2gn_0)}= \sqrt{{\cal L}^2_k - {\cal M}^2}
\end{eqnarray}
同様にして, 
\begin{eqnarray}
...
相互作用項は
\begin{eqnarray}
\nonumber  \ev{\varphi_{ex}^\dagger\varphi_{ex}^\dagger\varphi_{ex}\varphi_{ex}}_{ex} &=& \frac{1}{(2\pi)^6}\int d\bk_1\int d\bk_2\int d\bk_3\int d\bk_4\\
...
\begin{eqnarray}
  ...
  &=& \frac{1}{(2\pi)^6}\qty[2\qty(I_u'^{, 2} + I_v'^{+, 2}) + \qty(I_{uv} + I_{uv}^{+})^2+ 4I_u'I_v'^+]\label{Interaction-Exp}
\end{eqnarray}
となる. ただし
\begin{eqnarray}
\nonumber I_u' &=& \int d\bk n_{\bk} u_{\bk}^2 e^{2i\bk\cdot\bx}\hspace{1cm}I_v' = \int d\bk n_{\bk} v_{\bk}^2 e^{2i\bk\cdot\bx}\hspace{1cm} I_{uv} = \int d\bk n_{\bk} u_{\bk}v_{\bk}\\
\nonumber I_{uv}^+ &=& \int d\bk (1 + n_{\bk}) u_{\bk}v_{\bk} \hspace{1cm}I_v'^+ = \int d\bk (1 + n_{\bk}) v_{\bk}^2 e^{2i\bk\cdot\bx}\hspace{1cm}I_u'^+ = \int d\bk (1 + n_{\bk}) u_{\bk}^2 e^{2i\bk\cdot\bx}\\
\end{eqnarray}
であり, かつ$I_u'I_v'^+ = I_u'^+I_v'$であることを用いている.

以上で具体的にゼロモード・励起モードの期待値が計算できたので, (\ref{Hamiltonian-Exp})に(\ref{Zeromode-Exp}), (\ref{BdG-Exp}), (\ref{Interaction-Exp})を代入する. (\ref{Zeromode-Exp}), (\ref{BdG-Exp})については, $\bx$依存性を持たないので空間積分は系の体積$V$が出てくるだけだが, (\ref{Interaction-Exp})は$I'$系の項が$e^{2i\bk\cdot\bx}$を持っているのでその限りではない. $I'$を含む項の空間積分を実行する:
\begin{eqnarray}
  \int d\bx I_n'^2 &=& \int d\bk_1d\bk_2 n_{\bk_1}n_{\bk_2}u^2_{\bk_1}u^2_{\bk_2}\delta(\bk_1 + \bk_2)\\
  &=& \int d\bk_1 n_{\bk_1}n_{-\bk_1}u^2_{\bk_1}u^2_{-\bk_1}\\
  &=& 4\pi\int_{+0}^\infty dk\ k^2 n_{k}^2u^4_{k}
\end{eqnarray}
同様にして
\begin{eqnarray}
  \int d\bx I_v'^{+, 2} &=& 4\pi\int_{+0}^\infty dk\ k^2(1+n_k)^2v_k^4\\ \int d\bx I_u'I_v'^{+} &=& 4\pi\int_{+0}^\infty dk\ k^2 n_k(1+n_k)u_k^2v_k^2\hspace{1cm}\\
  %\int d\bx I_{uv}I_{uv}^+ &=& (4\pi)^2V\int_{+0}^\infty dk_1\int_{+0}^\infty dk_2 k_1^2 k_2^2n_{k_1}(1+n_{k_2})u_{k_1}v_{k_1}u_{k_2}v_{k_2}\\
  %&=& (4\pi)^2V\qty[\qty(\int_{+0}^\infty dk n_ku_kv_k)^2 + \qty(\int_{+0}^\infty dk_1 n_ku_{k_1}v_{k_1}\int_{+0}^\infty dk_2 u_{k_2}v_{k_2})]\\
  %\int d\bx I_{uv}^2     &=& V\qty(4\pi \int_{+0}^\infty dk\ k^2 n_ku_kv_k)^2\\
  %\int d\bx I_{uv}^{+, 2}&=& V\qty(4\pi \int_{+0}^\infty dk\ k^2 (1 + n_k)u_kv_k)^2
  \int d\bx (I_{uv} +  I_{uv}^+)^2     &=& V\qty(4\pi \int_{+0}^\infty dk\ k^2 (1 + 2n_k)u_kv_k)^2
\end{eqnarray}

まとめると, (\ref{Hamiltonian-Exp})は
\begin{eqnarray}
  \nonumber  \ev{H_{z, ex}}_0 =&& \int d\bx\Bigl[-\frac{2}{(2\pi)^2}\qty(-\xi^2\ev{Q^2}_z + \eta^2\ev{P^2}_z)\int_{+0}^\infty dk\frac{k^2\calM}{\omega_k}\qty(2n_k + 1)\\
...
\end{eqnarray}
前述の通り, $I'$系以外の空間積分は系の体積$V$が出てくるだけ.
\subsection{ふたたび熱力学ポテンシャルの具体形}
期待値が$s$時間依存しないことを用いるとZeromode-BdGカップリングの摂動一次まで取り入れた熱力学ポテンシャルは以下の通り:
\begin{eqnarray}
  \Omega &=& \Omega_0 -\frac{1}{\beta}\ln\ev{T\exp[-\int_0^\beta ds H_{z, ex}(s)]}_0\\
  &\simeq& \Omega_0 - \frac{1}{\beta}\ln(1 - \int_0^\beta ds\ev{H_{z, ex}(s)}_0)\\
  &=& \Omega_0  + \ev{H_{z, ex}}_0
\end{eqnarray}
これで計算の準備が整った.
\newpage
\section{数値計算}
...
左の自己無撞着方程式が収束すると$\xi, \eta, \ket{\Psi}$が決まり, それによって分配関数$Z$や, 熱力学ポテンシャル$\Omega$を(摂動的に)求めることが可能になる. $\Omega$の摂動展開によって得られた熱力学関数はZeromode-BdG間の相互作用を一部取り入れたものになっている.

\ul{$\Omega$の摂動一次の寄与を再び左側のループに反映させる必要があるのか, またそれがどのように取り入れればよい}\\\ul{のかがよくわかっていません.}
\begin{comment}
  \ul{これを取り入れることによって上}\\\ul{の自己無撞着ループは修正を受けるのだろうか？}もし修正を受けるのであればこの自己無撞着ループに$\Omega$も加わることになり, $\Omega$が収束するまでループを回す必要がある. \ul{この場合どうやってZeromode-BdGカップリングの寄}\\\ul{与を取り入れればよいのか？}
  
  ここで熱力学ポテンシャル$\Omega$はZeromode-BdGカップリングの一次まで取り入れて計算する. \ul{繰り返しになり}\\\ul{ますが, $\Omega$の摂動一次の寄与を再び左側のループに反映させる必要があるのか, またそれがどのように取り込まれ}\\\ul{るのかがよくわかっていません.}
\end{comment}
\newpage
