...
\nonumber  \therefore \ev{T\exp[-\int_0^\beta ds H_{z, ex}(s)]}_0 &\simeq& \ev{I -\int_0^{\beta}ds H_{z, ex}(s)}_0 =  1 - \int_0^\beta ds\ev{H_{z, ex}(s)}_0\label{Perturbed-thermo3}\\
...
\begin{eqnarray}
  \nonumber  \ev{H_{z, ex}}_0 &=& \ev{\varphi_z^\dagger(s)\varphi_z^\dagger(s)}_z\ev{\varphi_{ex}(s)\varphi_{ex}(s)}_{ex} + 4\ev{\varphi_z^\dagger(s)\varphi_z(s)}_z\ev{\varphi_{ex}^\dagger(s)\varphi_{ex}(s)}_{ex}\\
  &+& \ev{\varphi_z(s)\varphi_z(s)}_z\ev{\varphi_{ex}^\dagger(s)\varphi_{ex}^\dagger(s)}_{ex} + \ev{\varphi_{ex}^\dagger(s)\varphi_{ex}^\dagger(s)\varphi_{ex}(s)\varphi_{ex}(s)}_{ex}\label{Hamiltonian-Exp}
\end{eqnarray}
% ---------- Comment out -------------
\begin{comment}
\begin{eqnarray}
...
\begin{eqnarray}
  \ev{\varphi^\dagger_z\varphi^\dagger_z}_z &=& \ev{(-i\xi Q + \eta P)^\dagger(-i\xi Q + \eta P)^\dagger}_z = -\xi^2\ev{Q^2}_z + \eta^2\ev{P^2}_z\\
\ev{\varphi_z^\dagger\varphi_z}_z &=& \xi^2\ev{Q^2}_z + \eta^2\ev{P^2}_z\\
\ev{\varphi_z\varphi_z}_z &=& -\xi^2\ev{Q^2}_z + \eta^2\ev{P^2}_z
\end{eqnarray}
一方で励起部は
\begin{eqnarray}
  ...
  &=& \frac{1}{(2\pi)^3}\int d\bk_1 \qty(1+2n_{\bk})u_{\bk_1}v_{\bk_1} = \frac{1}{(2\pi)^3}\int_0^\infty dk\int_0^\pi d\theta\int_0^{2\pi} d\phi \qty(1+2n_k)u_kv_k\\
  &=& -\frac{1}{(2\pi)^2}\int_0^\infty dk\frac{k^2\calM}{\omega_k}\qty(2n_k + 1)
\end{eqnarray}
となる. この計算では
\begin{itemize}
\item[1. ]$v_{\bk} = v_k, n_{\bk} = n_k$であること
...
\item[4. ]$\int_{-\infty}^{\infty} d\bk\ev{a^\dagger_{-\bk}a_{-\bk}}_{ex} = \int_{-\infty}^{\infty} d\bk\ev{a^\dagger_{\bk}a_{\bk}}_{ex} = n_k$であること
\end{itemize}
などを用いている. $u_k, v_k, n_k, \calL_k...$などは2015夏の学校の資料を参照のこと. 同様にして
\begin{eqnarray}
...
相互作用項は
\begin{eqnarray}
  \ev{\varphi_{ex}^\dagger\varphi_{ex}^\dagger\varphi_{ex}\varphi_{ex}}_{ex} = 
\end{eqnarray}
\newpage
